...
QPixmap buildBlackPixmap(const QSize& size) {
    QImage image(size, QImage::Format_RGB32);
    image.fill(Qt::black);
    return QPixmap::fromImage(image);
}
...
void ProjectionWindow::showBlackFrame() {
    contentMode_ = ContentMode::Black;
...
QString ProjectionWindow::currentStatusText() const {
    switch (contentMode_) {
    case ContentMode::Black:
        return QStringLiteral("当前投影: 黑屏");
    case ContentMode::SiemensStar:
...
void ProjectionWindow::renderCurrentContent() {
    ...
    switch (contentMode_) {
    case ContentMode::Black:
        currentPixmap_ = buildBlackPixmap(targetSize);
        break;
    case ContentMode::SiemensStar:
